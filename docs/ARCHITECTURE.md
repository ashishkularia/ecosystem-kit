...
| `gh` (GitHub CLI) | `~/.local/bin` | not kit-shipped, but a machine-layer **prerequisite**: the fallback path for PR and issue work when the GitHub MCP server is not connected. Without it a session that needs to open a PR has neither route. `bootstrap-machine.sh` carries the no-sudo install step and `verify_gh`; it authenticates from the same `~/.secrets/github-pat` the other tools use, so no second credential |
