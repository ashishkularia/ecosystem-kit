...
| `artifacts` | object | `{"enabled": true, "dir": "docs/artifacts", "commit": true, "commit_type": "docs"}` | Controls `artifact_sync`, which mirrors every published artifact into the repo on `PostToolUse:Artifact`. `dir` is repo-relative and holds one directory per artifact: the **source file verbatim** (publish from this), a **readable digest** for HTML sources, a generated **`index.html`** (the browser-openable copy — the source is a fragment with no HTML skeleton, so it is not viewable alone), and `artifact.json`. A generated gallery at `<dir>/index.html` makes the tree a static site with no build step (`npx serve <dir>`). Generated files are banner-marked; the kit is stdlib-only, so md→html renders a documented subset and html→md is a lossy text digest. Idempotent on the artifact's stable `artifact_id`, so republishing rewrites its directory rather than adding one. `commit` makes a **pathspec** commit scoped to the artifact paths only — it ignores whatever else is staged, refuses on a `protected_branches` name or mid-merge/rebase, and never pushes. Set `enabled: false` in repos where artifacts should not be versioned. |
